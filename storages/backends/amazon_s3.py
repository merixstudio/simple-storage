...
class AmazonS3Storage(Storage):
    ...
    def __init__(self, aws_access_key_id: str, aws_secret_access_key: str, bucket_name: str):
        self._session: Session = boto3.Session(aws_access_key_id=aws_access_key_id, aws_secret_access_key=aws_secret_access_key)
        self._s3: ServiceResource = self._session.resource(service_name=self._SERVICE_NAME)
    ...
    def write(self, name: str, content: AnyStr, mode: str = "a"):
        self._bucket.put_object(Body=content) # TODO
    ...
    def exists(self, name: str) -> bool:
        try:
            self._session.client(service_name=self._SERVICE_NAME).head_object(Bucket=self._bucket_name, Key=name)
            return True
